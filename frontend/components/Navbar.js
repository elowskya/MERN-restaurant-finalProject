...
const Navbar = () => {
  ...
  return (
    ...
    />
      ...
      <span className="ml-3 font-semibold text-sm text-black">Hello, {session.user?.email}</span>
      &nbsp;
      &nbsp;
      &nbsp;
      <button onClick={signOut} style={{border:'1.5px solid indigo', padding:'5px', borderRadius:'10px'}}> Sign Out</button>
      </div>
      </div>
  )
}
